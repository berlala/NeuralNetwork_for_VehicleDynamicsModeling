...
def plot_run(path_dict: dict,
    ...
    yaw_result = results[:, 1][:, np.newaxis]


    vx_label = labels[start:params_dict['Test']['run_timespan'] + start, 0][:, np.newaxis]
    yaw_label = labels[start:params_dict['Test']['run_timespan'] + start, 1][:, np.newaxis]
    ...
    scaler_temp_result = np.concatenate((vx_result, yaw_result ), axis=1)
    scaler_temp_label = np.concatenate((vx_label, yaw_label ), axis=1)
    ...
    yaw_result_scaled = scaler_temp_result[:, 1]


    vx_label_scaled = scaler_temp_label[:, 0]
    yaw_label_scaled = scaler_temp_label[:, 1]
    ...
    data = np.asarray([mean_squared_error(yaw_label, yaw_result),
                       ...
                       mean_absolute_error(vx_label, vx_result)]).reshape(2, 2).round(round_digits)

    column_header = ['yaw rate', 'long. vel. vx']
    row_header = ['MSE', 'MAE']
    ...
    data = np.asarray([mean_squared_error(yaw_label_scaled, yaw_result_scaled),
                       ...
                       mean_absolute_error(vx_label_scaled, vx_result_scaled)]).reshape(2, 2).round(round_digits)
